...
def index():
    ...
    SELECT 
        ...
        (SELECT data FROM dane d2 WHERE d2.ticker = d.ticker AND d2.data < d.data ORDER BY d2.data DESC LIMIT 1) as data_m1,
        (SELECT data FROM dane d3 WHERE d3.ticker = d.ticker AND d3.data < (SELECT data FROM dane d4 WHERE d4.ticker = d.ticker AND d4.data < d.data ORDER BY d4.data DESC LIMIT 1) ORDER BY d3.data DESC LIMIT 1) as data_m2
    FROM obroty o
    ...
    for r in rows:
        ...
        # Przekazujemy kurs waluty z NBP do wiersza dla modala
